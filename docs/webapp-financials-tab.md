...
- 2026-09-22：修复 fin 永不赋值（financials 复用共享 seq 守卫被 loadAnalysis/loadDcf 推进后永假，改为独立 finSeq + 并行请求，e0f2cd6）；金额双重除修复实测生效（442.8 亿$）
- 2026-09-22：DCF 基期信息改结构化表格（项目/金额/说明，ttm 新增 notes 字典，components 保留兼容）+ 新增每股估值段（取最新 DCF 报告 valuation：每股内在价值/25%/50% 安全边际/报告现价/溢价折价）
